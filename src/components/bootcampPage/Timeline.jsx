          ...
          <div className="flex flex-col items-center justify-center">
            {timelineData.map((timeline, index) => (
              <div className="w-full sm:w-[80%]">
                <div key={index} className="w-full flex items-center mb-8 sm:mb-12 justify-center sm:justify-none">
                  {index % 2 === 0 ? (
                    <>
                      {/* Left Side (Title) */}
                      <div className="w-1/2 sm:w-1/3 flex justify-end px-4 sm:px-8 animate-fadeInRight">
                        <div className="border-2 rounded-[20px] p-4 sm:p-8 w-full">
                          {timeline.events.map((event, index) => (
                            <div key={index} className="flex items-center justify-between my-4 sm:my-2 flex-col sm:flex-row">
                      ...
                      {/* Right Side (Date) */}
                      <div className="w-1/2 sm:w-1/3 flex justify-start px-4 sm:px-8 animate-fadeInLeft">
                    ...
                    <>
                      {/* Left Side (Date) */}
                      <div className="w-1/2 sm:w-1/3 flex justify-end px-4 sm:px-8 animate-fadeInLeft">
                        <div className="w-full sm:w-[65%] text-center font-bold bg-[#F0F0F0] p-4 rounded-[25px] text-base sm:text-2xl">
                          {timeline.date}
                        </div>
                      </div>
                      ...
                      {/* Right Side (Title) */}
                      <div className="w-1/2 sm:w-1/3 flex justify-start px-4 sm:px-8 animate-fadeInRight">
                        <div className="border-2 rounded-[20px] p-4 sm:p-8 w-full">
                          {timeline.events.map((event, index) => (
                            <div key={index} className="flex items-center justify-between my-4 sm:my-2 flex-col sm:flex-row">
                              <div className="flex w-full flex-col sm:flex-row justify-start">
